...
def intro_screen():
    ...
    sety(-270)
    pendown()
    write("Made by Soujash Banerjee!",True,font=("Algerian",16,"bold italic"))
...
def send_pressed():
        ...
        if input_get.split()[0]=="play" and input_get.split()[1]!="music":
            MusicWindow(inpur_get)
        print(input_get,cq)
        var = StringVar()
        root.option_add('*Font', 'Algerian','100')
        label0 = Label( textvariable=var, relief=FLAT ,bg="blue",foreground="white")
        var.set(response(input_get))
        speak(response(input_get))
        label0.pack_propagate(True)
        label0.pack()
        cqp=200 +90*cq+40
        label0.place(x=0,y=cqp)
        label0r = Label(text=input_get,relief=FLAT,bg="gray",foreground="white")
        input_user.set('')
        label0r.pack()
        crp=10 +90*cq+120
        print(cqp,crp)
        label0r.place(x=560,y=crp)
        respo.append(response(input_get))
        inp.append(input_get)
        
        return "break"
        win_width, win_height, bg_color = 1000, 1000, 'black'
...
# Adjust size 
root.geometry("703x598") 

# Add image file 
bg = PhotoImage(file = "SAGNIK_BG(dark).png") 
...
# Create Canvas 
    canvas1 = Canvas(window, width = 793, 
    				height = 668) 
    ...
    def enter_pressed(event):
        ...
        if input_get=="music":
            var = StringVar()
            label = tkinter.Label( background="gray",textvariable=var, relief=RAISED )
            var.set("Playing...")
            speak("Playing...")
            input_user.set('')
            label.pack()
